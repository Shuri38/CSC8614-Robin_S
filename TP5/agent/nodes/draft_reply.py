...
# NOTE: modifiez ce port selon votre config Ollama (local ou serveur)
PORT = "11434"
...
def evidence_to_context(evidence: List[EvidenceDoc]) -> str:
    """
    Transforme la liste d'evidence en un contexte citables pour le LLM.
    """
    blocks = []
    for d in evidence:
...
def safe_mode_reply(state: AgentState, reason: str) -> str:
    """
    Réponse prudente si pas d’evidence ou erreurs de parsing.
    """
    return (
        "Je n’ai pas suffisamment d’informations pour répondre précisément à votre email. "
        "Pouvez-vous préciser votre demande ou fournir des documents pertinents ?"
    )


def call_llm(prompt: str) -> str:
    """
    Appelle le LLM Ollama et retourne la réponse texte.
    """
    llm = ChatOllama(base_url=f"http://127.0.0.1:{PORT}", model=LLM_MODEL)
    resp = llm.invoke(prompt)
    # Supprime toute balise <think> générée
    return re.sub(r"<think>.*?</think>\s*", "", resp.content.strip(), flags=re.DOTALL).strip()


def draft_reply(state: AgentState) -> AgentState:
    """
    Nœud qui produit draft_v1 à partir des evidences.
    """
    ...
    # Cas où il n'y a pas d'evidence
    ...
    # Construire contexte à partir des evidences
