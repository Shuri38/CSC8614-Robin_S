...
# Port Ollama (cohérent avec les autres nœuds)
PORT = "11434"
...
def rewrite_query(state: AgentState) -> AgentState:
    ...
    # requête initiale : soit celle décidée, soit un fallback sur le corps
    ...
    # fallback défensif : on garde l’ancienne requête
    if not q2:
        q2 = q1
        state.add_error("rewrite_query: empty rewrite")

    # mise à jour de la requête (UNE seule proposition)
    ...
    log_event(
        ...
        "node_end",
        {
            "node": "rewrite_query",
            "status": "ok",
            "q2": state.decision.retrieval_query,
        },
    )
    return state
