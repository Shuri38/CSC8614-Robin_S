...
{{ "intent": "...", "category":"...", "priority":1, "risk_level":"...", "needs_retrieval":true, "retrial_query":"...", "rationale":"..." }}
...
def classify_email(state: AgentState) -> AgentState:
    ...
    # --- Pré-check heuristique prompt injection ---
    low = state.body.lower()
    if any(x in low for x in ["ignore previous", "system:", "tool", "call", "ex filtrate"]):
    ...
    try:
        ...
        log_event(
            state.run_id,
            "error",
            {"node": "classify_email", "kind": "parse_or_validation", "msg": str(e)},
        )
