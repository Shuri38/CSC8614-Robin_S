...
def load_one_email(path: str) -> Dict[str, str]:
    ...
    # Champs extraits (ou chaîne vide si absent)
    email_id = email_id_match.group(1) if email_id_match else ""
    subject = subject_match.group(1) if subject_match else ""
    from_ = from_match.group(1) if from_match else ""
...
if __name__ == "__main__":
    ...
    for e in emails:
        print(f"- {e['email_id']}: {e['subject']} {e['body']} ({os.path.basename(e['path'])})")
